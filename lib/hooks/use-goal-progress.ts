// lib/hooks/use-goal-progress.ts
"use client"

import { useCallback } from "react"
import { supabase } from "@/lib/supabaseClient"
import { differenceInDays, parseISO, format, startOfDay, subDays, startOfMonth, endOfMonth, startOfYear, endOfYear } from "date-fns"
  ...
  const calculateProgress = useCallback(async (goal: Goal, userId: string): Promise<{
    ...
    switch (goal.track_source) {
      ...
      case "finance_savings":
        const savings = await calcFinance(userId, "savings", startDate, config.category_id)
        return { currentValue: savings }

      case "finance_income":
        const income = await calcFinance(userId, "income", startDate, config.category_id)
        return { currentValue: income }

      case "finance_expense":
        const expense = await calcFinance(userId, "expense", startDate, config.category_id)
    ...
    switch (goal.track_source) {
      ...
      case "finance_expense": {
        ...
        if (!financeType || financeType === "income" || goal.track_source === "finance_savings") {
          const { data: incomeData } = await supabase
            ...
            .gte("date", startDate)
            .lte("date", endDate)
          incomeTotal = incomeData?.reduce((sum, r) => sum + (r.amount || 0), 0) || 0
        }
        ...
        if (!financeType || financeType === "expense" || goal.track_source === "finance_savings") {
          const { data: expenseData } = await supabase
            ...
            .gte("date", startDate)
            .lte("date", endDate)
          expenseTotal = expenseData?.reduce((sum, r) => sum + (r.amount || 0), 0) || 0
        }
      ...
      case "exercise_minutes": {
        const { data: exerciseData } = await supabase
          ...
          .gte("date", startDate)
          .lte("date", endDate)
        return exerciseData?.reduce((sum, r) => sum + (r.duration_minutes || 0), 0) || 0
      }
