...
import { NOTE_CATEGORIES, type UnitNote, type NoteCategoryType } from "@/types/custom"
...
export function NoteDialog({ open, onOpenChange, unitId, note, onSaved }: NoteDialogProps) {
  ...
  const handleSave = async () => {
    ...
    if (note) {
      ...
      const newOrder = ((maxOrderData?.order as number) ?? -1) + 1
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select value={category} onValueChange={(v) => setCategory(v as NoteCategoryType)}>
                ...
                <SelectContent>
                  {NOTE_CATEGORIES.map((cat: { value: string; label: string }) => (
